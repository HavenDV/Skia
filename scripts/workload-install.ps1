...
<#
.SYNOPSIS
Installs Skia workload manifest.
.DESCRIPTION
Installs the WorkloadManifest.json and WorkloadManifest.targets files for Skia to the dotnet sdk.
.PARAMETER Version
...
function Install-Pack([string]$Id, [string]$Version, [string]$Kind, [string]$Source) {
    ...
    switch ($Kind) {
        "manifest" {
            ...
            New-Item -Path $SkiaManifestDir -ItemType "directory" -Force | Out-Null
            Copy-Item -Path "$TempUnzipDir\data\*" -Destination $SkiaManifestDir -Force
        }
...
function Remove-Pack([string]$Id, [string]$Version, [string]$Kind) {
    switch ($Kind) {
        "manifest" {
            Remove-Item -Path $SkiaManifestDir -Recurse -Force
        }
        {($_ -eq "sdk") -or ($_ -eq "framework")} {
            $TargetDirectory = $(Join-Path -Path $DotnetInstallDir -ChildPath "packs\$Id\$Version")
...
function Install-SkiaWorkload([string]$DotnetVersion, [string]$Source)
{
    ...
    if (Test-Path $SkiaManifestFile) {
        $ManifestJson = $(Get-Content $SkiaManifestFile | ConvertFrom-Json)
        $OldVersion = $ManifestJson.version
        if ($OldVersion -eq $Version) {
            $DotnetWorkloadList = Invoke-Expression "& '$DotnetCommand' workload list | Select-String -Pattern '^skia'"
            if ($DotnetWorkloadList)
            {
                Write-Host "Skia Workload $Version version is already installed."
                Continue
            }
        }
    ...
    New-Item -Path $(Join-Path -Path $DotnetInstallDir -ChildPath "metadata\workloads\$DotnetVersionBand\InstalledWorkloads\skia") -Force | Out-Null
    if (Test-Path $(Join-Path -Path $DotnetInstallDir -ChildPath "metadata\workloads\$DotnetVersionBand\InstallerType\msi")) {
        New-Item -Path "HKLM:\SOFTWARE\Microsoft\dotnet\InstalledWorkloads\Standalone\x64\$DotnetTargetVersionBand\skia" -Force | Out-Null
    }

    # Clean up
    Remove-Item -Path $TempDir -Force -Recurse

    Write-Host "Done installing Skia workload $Version"
}
...
{
    Write-Host "`n.NET SDK version 6 or later is required to install Skia Workload."
}
else
{
